...
public class ADDSimple<V> {
	 ...
	 * @param varOrder
	 */
	public ADDSimple(HashMap<Integer,Node> nodes, Node root, ArrayList<V> varOrder) {
		super();
		this.nodes = nodes;
		this.root = root;
	 ...
	 */
	public DAGSimple createDAG() throws CloneNotSupportedException {
		
		// Création de la table de Hash qui serviront à stocker les noeuds du DAG
		...
		if (rootDag.isLeaf()) {
			rootDag = dagNodes.get(this.getNbNodes());
		}
		else {
		...
		DAGSimple res = new DAGSimple(dagNodes,rootDag,this.varOrder);
	 ...
	 * @throws CloneNotSupportedException
	 */
	private int fun(HashMap<Integer,Node> dagNodes, Node node, int depth, int nbNode) throws CloneNotSupportedException {
